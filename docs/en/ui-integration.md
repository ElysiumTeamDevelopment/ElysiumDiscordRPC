...
Guide for integrating ElysiumDiscordRPC into RenPy game user interfaces.

## 📋 Overview

ElysiumDiscordRPC provides flexible integration options:

- 🎛️ **Ready UI** - built-in settings screen (`discord_rpc_settings.rpy`)
- 🔧 **Custom integration** - functions for creating custom interface
- 📊 **Status monitoring** - connection status display
- ⚡ **Quick actions** - enable/disable buttons

## 🎛️ Using Built-in UI

### Connecting Ready Screen

If you're using `discord_rpc_settings.rpy`, add to main menu:
