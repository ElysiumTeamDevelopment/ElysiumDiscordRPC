...
Complete reference for all functions and methods of the ElysiumDiscordRPC module.

## 📋 API Overview

ElysiumDiscordRPC provides several API levels:

- 🎮 **Simple functions** - `discord_set_*()` for quick usage
- 🔧 **DiscordRPCAPI class** - `drpc.*()` for advanced usage
- ⚙️ **Main class** - `discord_rpc.*()` for full control
- 📊 **Helper functions** - configuration utilities

## 🎮 Simple Functions

### discord_set_main_menu()
Sets main menu status.

```python
$ discord_set_main_menu()
```

**Uses template:** `discord_config.main_menu_presence`

### discord_set_in_game(chapter=None, character=None)
Sets gameplay status.

```python
$ discord_set_in_game()                           # Basic status
$ discord_set_in_game("Chapter 1")                # With chapter
$ discord_set_in_game("Chapter 1", "Eileen")      # With chapter and character
$ discord_set_in_game(character="Alice")          # Character only
```

**Parameters:**
- `chapter` (str, optional) - chapter/scene name
- `character` (str, optional) - character name

### discord_set_dialogue(character=None, scene=None)
Sets dialogue reading status.

```python
$ discord_set_dialogue()                          # Basic status
$ discord_set_dialogue("Eileen")                  # With character
$ discord_set_dialogue("Eileen", "Room")          # With character and scene
$ discord_set_dialogue(scene="Park")              # Scene only
```

**Parameters:**
- `character` (str, optional) - speaking character name
- `scene` (str, optional) - current scene name

### discord_set_menu(menu_name="Menu")
Sets menu navigation status.

```python
$ discord_set_menu()                              # "In menu: Menu"
$ discord_set_menu("Settings")                    # "In menu: Settings"
$ discord_set_menu("Save")                        # "In menu: Save"
```
...
**Uses template:** `discord_config.paused_presence`
...
```python
$ discord_set_custom("My status")                            # State only
$ discord_set_custom("Playing", "Chapter 1")                 # State + details
$ discord_set_custom("Battle", "Final boss",
                     large_image="battle_bg")                # With additional parameters
```

**Parameters:**
- `state` (str) - main status text
- `details` (str, optional) - additional text
- `**kwargs` - any additional Discord RPC parameters
